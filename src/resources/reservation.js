...
class Reservation extends Train {
  ...
  get buyLimit() {
    if (!this.isWaiting) {
      return new Date(
        ...getParsedDate(
          `${this.buyLimitDate.substr(0, 4)}-${this.buyLimitDate.substr(
            4,
            2
          )}-${this.buyLimitDate.substr(6, 2)} ` +
            `${this.buyLimitTime.substr(0, 2)}:${this.buyLimitTime.substr(
              2,
              2
            )}:${this.buyLimitTime.substr(4, 2)}`
        )
      );
    } else {
      return null;
  ...
  toString() {
    ...
    const buyLimitTime = `${this.buyLimitTime.substr(
      0,
      2
    )}:${this.buyLimitTime.substr(2, 2)}`;
    const buyLimitDate = `${this.buyLimitDate.substr(
      4,
      2
    )}월 ${this.buyLimitDate.substr(6)}일`;
