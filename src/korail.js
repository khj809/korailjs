...
import { reducePassengers } from "./helpers";
import {
  KorailError,
  NeedToLoginError,
  NoResultsError,
...
class Korail {
  ...
  searchTrain = (
    ...
    const adultCount = passengers
      .filter(psgr => psgr instanceof AdultPassenger)
      .reduce((acc, psgr) => acc + psgr.count, 0);
    const childCount = passengers
      .filter(psgr => psgr instanceof ChildPassenger)
      .reduce((acc, psgr) => acc + psgr.count, 0);
    const seniorCount = passengers
      .filter(psgr => psgr instanceof SeniorPassenger)
      .reduce((acc, psgr) => acc + psgr.count, 0);
  ...
  reserve = (
    ...
    if (
      reserveOption === ReserveOptions.GENERAL_FIRST ||
      reserveOption === ReserveOptions.GENERAL_ONLY
    ) {
      seatType = "1";
    } else if (
      reserveOption === ReserveOptions.SPECIAL_FIRST ||
      reserveOption === ReserveOptions.SPECIAL_ONLY
    ) {
      seatType = "2";
    }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const resp = await axios.get(KORAIL_TICKETRESERVATION, {
          params,
          timeout
        });
        const data = resp.data;
